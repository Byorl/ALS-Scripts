...
local function loadConfig()
    ...
    if isfile(configPath) then
        ...
        if ok and type(data) == "table" then
            ...
            data.macroMaps = data.macroMaps or {}
            return data
        end
    end
    return { toggles = {}, inputs = {}, dropdowns = {}, abilities = {}, autoJoin = {}, macroMaps = {} }
end
...
getgenv().CurrentMacro = getgenv().Config.inputs.CurrentMacro or nil
getgenv().MacroRecording = false
getgenv().MacroPlaying = getgenv().Config.toggles.MacroPlayToggle or false
getgenv().MacroData = {}
getgenv().MacroStepDelay = tonumber(getgenv().Config.inputs.MacroStepDelay) or 0
getgenv().MacroResumeFromStep = 0
...
local function cacheMacroTowerInfo()
    ...
    for _, mod in pairs(towerInfoPath:GetChildren()) do
        if mod:IsA("ModuleScript") then
            ...
            if ok then getgenv().MacroTowerInfoCache[mod.Name] = data end
        end
    end
    print("[Macro] Cached tower info for " .. tostring(#getgenv().MacroTowerInfoCache) .. " towers")
end

local function cacheMacroRemotes()
    if next(getgenv().MacroRemoteCache) then return end
    for _, v in pairs(RS:GetDescendants()) do
        if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") then
            getgenv().MacroRemoteCache[v.Name:lower()] = v
        end
    end
    print("[Macro] Cached " .. tostring(#getgenv().MacroRemoteCache) .. " remotes")
end
...
local macroCashTrackingActive = false
local function trackMacroCash()
    if macroCashTrackingActive then return end
    macroCashTrackingActive = true
...
local function getRecentMacroCashDecrease(withinSeconds)
...
local function getMacroPlaceCost(towerName)
    cacheMacroTowerInfo()
    if not getgenv().MacroTowerInfoCache[towerName] then return 0 end
...
local function loadMacros()
    ...
    for _, file in pairs(listfiles(MACRO_FOLDER)) do
        if file:sub(-5) == ".json" then
            ...
            if ok then getgenv().Macros[file:match("([^/\\]+)%.json$")] = data end
        end
    end
end
...
local function saveMacroPlaybackState(macroName, currentStep)
    pcall(function()
        local state = {
            macro = macroName,
            step = currentStep,
...
local function loadMacroPlaybackState()
    local state = nil
    pcall(function()
...
local function clearMacroPlaybackState()
    pcall(function()
        if isfile(STATE_FILE) then
            delfile(STATE_FILE)
        end
...
local Tabs = {
    ...
    MacroControls = MacroSection:Tab({ Title = "Play/Record", Icon = "play-circle" }),
    MacroMaps = MacroSection:Tab({ Title = "Macro Maps", Icon = "map-pin" }),
    MacroStatus = MacroSection:Tab({ Title = "Status", Icon = "activity" }),
...
Tabs.MacroControls:Paragraph({
    Title = "🎬 Macro System",
    Desc = "Record and playback tower placement and upgrade sequences automatically"
})
Tabs.MacroControls:Space()

Tabs.MacroControls:Section({ Title = "📝 Macro Management" })

local macroDropdown = Tabs.MacroControls:Dropdown({
    Flag = "MacroSelect",
    Title = "Select Macro",
    Values = getMacroNames(),
    Value = getgenv().CurrentMacro,
    Callback = function(v)
        getgenv().CurrentMacro = v
        getgenv().Config.inputs.CurrentMacro = v
        saveConfig(getgenv().Config)
        if v and getgenv().Macros[v] then
            getgenv().MacroData = getgenv().Macros[v]
            getgenv().MacroTotalSteps = #getgenv().MacroData
            notify("Macro Selected", v .. " (" .. #getgenv().MacroData .. " steps)", 3)
            updateMacroStatus()
        end
    end,
    Searchable = true,
...
Tabs.MacroControls:Input({
    Flag = "MacroName",
    Title = "Create New Macro",
    Placeholder = "Enter macro name",
    Callback = function(v)
        if v and v ~= "" and not getgenv().Macros[v] then
...
Tabs.MacroControls:Section({ Title = "⚙️ Playback Settings" })

Tabs.MacroControls:Input({
    Flag = "MacroStepDelay",
    Title = "Step Delay (seconds)",
    Value = tostring(getgenv().MacroStepDelay),
    Placeholder = "0",
    Callback = function(v)
        getgenv().MacroStepDelay = tonumber(v) or 0
        getgenv().Config.inputs.MacroStepDelay = tostring(getgenv().MacroStepDelay)
        saveConfig(getgenv().Config)
    end
})
...
Tabs.MacroMaps:Paragraph({
    Title = "🗺️ Dynamic Map Assignment",
    Desc = "Assign macros to specific maps - automatically switches based on gamemode and map!"
})

Tabs.MacroMaps:Space()

local selectedMacroGamemode = "Story"
local currentMacroMapSection = nil

local function updateMacroMapDisplay()
    if currentMacroMapSection then
        pcall(function()
            currentMacroMapSection:Destroy()
        end)
        currentMacroMapSection = nil
    end
    
    task.wait(0.05)
    
    local maps = getMapsByGamemode(selectedMacroGamemode)
    
    if #maps == 0 then
        currentMacroMapSection = Tabs.MacroMaps:Section({
            Title = "⚠️ No Maps Available",
            Opened = true,
        })
        currentMacroMapSection:Paragraph({
            Title = "No Maps Found",
            Desc = "No maps available for " .. selectedMacroGamemode
        })
        return
    end
    
    currentMacroMapSection = Tabs.MacroMaps:Section({
        Title = "📍 " .. selectedMacroGamemode .. " Maps",
        Opened = true,
    })
    
    for _, mapName in ipairs(maps) do
        local key = selectedMacroGamemode .. "_" .. mapName
        local currentMacro = getgenv().MacroMaps[key] or "--"
        
        local macroNames = getMacroNames()
        table.insert(macroNames, 1, "--")
        
        currentMacroMapSection:Dropdown({
            Flag = "MacroFor_" .. key,
            Title = mapName,
            Values = macroNames,
            Value = currentMacro,
            Callback = function(value)
                if value ~= "--" then
                    ...
                    getgenv().Config.macroMaps = getgenv().MacroMaps
                    saveConfig(getgenv().Config)
...
task.spawn(function()
    ...
    pcall(function()
        ...
        if gamemode and mapName then
            ...
            local key = gm .. "_" .. mn
            if getgenv().MacroMaps[key] and getgenv().MacroMaps[key] ~= "--" then
                getgenv().CurrentMacro = getgenv().MacroMaps[key]
                getgenv().MacroData = getgenv().Macros[getgenv().CurrentMacro] or {}
                getgenv().MacroTotalSteps = #getgenv().MacroData
                
                getgenv().Config.inputs.CurrentMacro = getgenv().CurrentMacro
                saveConfig(getgenv().Config)
                
                if macroDropdown then
                    pcall(function()
                        macroDropdown:Select(getgenv().CurrentMacro)
                    end)
                end
                    ...
                    if upgradeLevel > towerMonitor[towerName].lastLevel then
                        ...
                        if (now - towerMonitor[towerName].lastRecordTime) > 0.12 then
                            task.spawn(function()
                                ...
                                local cost = getRecentMacroCashDecrease(2.5)
                                ...
                                print("[Macro] ✓ Recorded Upgrade:", towerName, "Lv" .. levelBefore, "→", "Lv" .. upgradeLevel, "($" .. cost .. ")", "[Total:", #getgenv().MacroData .. "]")
                            end)
                        end
                    end
                end
                    ...
                    task.spawn(function()
                        local success, err = pcall(function()
                            ...
                            if countAfter > countBefore and countAfter <= placementLimit then
                                ...
                                local cost = getRecentMacroCashDecrease(2.5)
                                if cost == 0 then
                                    cost = getMacroPlaceCost(towerName)
                                end
                                ...
                                print("[Macro] ✓ Recorded Place:", towerName, "($" .. cost .. ")", countBefore, "→", countAfter, "/", placementLimit, "[Total:", #getgenv().MacroData .. "]")
                            end
...
if not isInLobby then
    task.spawn(function()
        local macroPlaybackActive = false
        
        while true do
            task.wait(1)
            
            if getgenv().MacroPlaying and not macroPlaybackActive and not getgenv().MacroRecording then
                pcall(function()
                    local gamemode = RS:FindFirstChild("Gamemode")
                    local mapName = workspace:FindFirstChild("Map") and workspace.Map:FindFirstChild("MapName")
                    if gamemode and mapName then
                        ...
                        if getgenv().MacroMaps[key] and getgenv().MacroMaps[key] ~= "--" then
                            ...
                            getgenv().MacroData = getgenv().Macros[getgenv().CurrentMacro] or {}
                            getgenv().MacroTotalSteps = #getgenv().MacroData
                            notify("Map Override", "Using " .. getgenv().CurrentMacro .. " for " .. mn, 3)
                        end
                    end
                end)
                
                if not getgenv().CurrentMacro or #getgenv().MacroData == 0 then
                    task.wait(5)
                else
                    macroPlaybackActive = true
                    
                    task.spawn(function()
                        local savedState = loadMacroPlaybackState()
                        if savedState and savedState.macro == getgenv().CurrentMacro and (tick() - savedState.timestamp) < 300 then
                            getgenv().MacroResumeFromStep = savedState.step
                            notify("Resuming Playback", getgenv().CurrentMacro .. " from step " .. getgenv().MacroResumeFromStep, 4)
                        else
                            getgenv().MacroResumeFromStep = 0
                            notify("Playback Started", getgenv().CurrentMacro, 3)
                        end
                        
                        local step = getgenv().MacroResumeFromStep > 0 and getgenv().MacroResumeFromStep or 1
                        local lastWave = 0
                        ...
                        if getgenv().MacroResumeFromStep == 0 then
                            ...
                            repeat task.wait(0.1) until not hasStartButton() or not getgenv().MacroPlaying
                            if not getgenv().MacroPlaying then 
                            ...
                            getgenv().MacroStatusText = "Resuming"
                            updateMacroStatus()
                            task.wait(0.5)
                        end
                        
                        while getgenv().MacroPlaying do
                            if step > #getgenv().MacroData then
                                getgenv().MacroStatusText = "Completed"
                                getgenv().MacroWaitingText = ""
                                getgenv().MacroActionText = ""
                                getgenv().MacroUnitText = ""
                                updateMacroStatus()
                            ...
                            local action = getgenv().MacroData[step]
                            ...
                            saveMacroPlaybackState(getgenv().CurrentMacro, step)
                            ...
                            while not actionSuccess and retryCount < maxRetries and getgenv().MacroPlaying do
                                local success = pcall(function()
                            ...
                            if getgenv().MacroStepDelay > 0 then
                                task.wait(getgenv().MacroStepDelay)
                            else
                                task.wait(0.06)
                            end
                        end
